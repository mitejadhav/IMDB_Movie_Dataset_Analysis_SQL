...
-- Following columns have null values: Country(20), worlwide_gross_income(3724), languages(194) and production_company(528)
...
FROM
    movie
GROUP BY year;
-- Most number of movies (3052) were released in 2017.
...
-- Most number of movies (824) were released in March.
...
-- Drama (4285) movies had the highest number of movies produced overall. 
...
-- Action genre has the highest average duration of 112.88 mins, then Romanance (109.53 mins), then Crime (107.05 mins) and so on with Horror being the lowest at 92.72 mins.
...
AS movie_count, 
RANK() OVER(ORDER BY count(genre) DESC) 
AS genre_rank
FROM genre
GROUP BY genre)
...
-- Thriller has a movie count of 1484 and is in top 3 among all genres in terms of number of movies.
...
-- Finding the top 10 movies based on avg_rating using the rank() and limit clause 
...
-- Using CTE with WHERE clause to find the top 10 movies based on the avg_rating
...
-- Now running the same query with DENSE_RANK() and using CTE with WHERE clause to find the top 10 movies based on the avg_rating
WITH top_ten_movies AS
(
SELECT title, avg_rating,
...
-- Summarising the ratings table based on the movie counts by median ratings using group by and order by. 
SELECT 
	median_rating,
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
-- Finding the production company that has most number of hit movies based on average rating given above 8 by voters
SELECT 
	production_company, 
	COUNT(movie_id) AS movie_count,
...
-- Following two production movies tied at rank 1 that has the most number of hits based on avg ratings more than 8
...
-- Finding movies exclusively released in USA in each genre during March 2017 and had more than 1,000 votes
SELECT 
	genre,
...
-- Finding movies released in USA in each genre during March 2017 and had more than 1,000 votes
SELECT 
	genre,
    Count(m.id) AS MOVIE_COUNT
FROM
...
+---------------+-------------------+---------------------+*/
-- Type your code below:
-- Group by function is used to avoid duplication of titles categorized under multiple genres. 
SELECT
	title,
    avg_rating,
    genre 
...
-- Query ran without using GROUP BY gives movies with rating above 8 present in multiple genres. 
SELECT
title,
avg_rating,
genre
...
-- There were total of 361 movies that were published between 1 April 2018 and 1 April 2019 and given a median rating of 8
...
-- German movies got appx (28745) 36.87% more votes than Italian movies
...
select
            ...
            sum(case when known_for_movies is null then 1 else 0 end) as known_for_nulls
from names;

-- Following columns have NULL values: height = 17335, date_of_birth = 13431 and known_for_nulls = 15226. Name doesnt have any NULLs
...
select n.name as director_name,
		...
		RANK() over(order by count(g.movie_id) desc) as director_row_rank
from names as n
...
from ratings as rt
inner join role_mapping as rlmp
on rlmp.movie_id = rt.movie_id
inner join names as nm
on rlmp.name_id = nm.id
...
-- Top 3 Genres based on most number of movies which are Top 3 Genres based on most number of movies
WITH top_genres AS
(	SELECT
	genre,
	COUNT(movie_id) as movie_count
...
-- we should convert worldwide_gross_income datatype from 'varchar' to decimal

-- we should convert values in INR to to dollars considering 1 USD = 81.42 INR (as per 04 dec 2022)
movie_income AS
(	SELECT g.genre, year, title AS movie_name,
		CASE
		WHEN worlwide_gross_income LIKE 'INR%' 
		THEN Cast(Replace(worlwide_gross_income, 'INR', '') AS DECIMAL(12)) / 81.42
...
-- five highest grossing movies for year 2017, 2018 and 2019 from top 3 genre
top_movies AS
(	SELECT *,
